...
class inimigos(): # Classe para definir movimento e imprirmir os inimigo
    ...
    def colisao_espada(self,espada_hitbox,dano):
        ...
        if self.hitbox[1] + self.hitbox[3] >= espada_hitbox[1] and self.hitbox[1] +espada_hitbox[3] <= espada_hitbox[1] + espada_hitbox[3] and self.cont>60:
            if self.hitbox[0] + self.hitbox[2] >= espada_hitbox[0] and self.hitbox[0] + espada_hitbox[2] <= espada_hitbox[0] + espada_hitbox[3]:
    ...
    def desenhar_inimigo_roberto(self,jogador_x,jogador_y):
        if self.vida_inimigo > 0:
            ...
            if jogador_x + 32 >= self.bixo_x:
                tela.blit(self.robertoi_direita, (self.bixo_x - 32, self.bixo_y - 32), (self.x_animacao_roberto, 0, 60, 44))
            if jogador_x + 32 < self.bixo_x:
                tela.blit(self.robertoi_esquerda, (self.bixo_x - 32, self.bixo_y - 32), (self.x_animacao_roberto, 0, 60, 44))
            pygame.draw.rect(tela,vermelho,self.hitbox,1)
    ...
    def desenhar_inimigo_jeorgia(self):
        if self.vida_inimigo > 0:
            ...
            pygame.draw.rect(tela,vermelho,self.hitbox,1)
            #tela.blit(self.jeorgiai_direita, (self.bixo_x - 26, self.bixo_y - 32), (self.x_animacao_jeorgia, 0, 44, 62))
            ...
            if self.bateu == True:
                tela.blit(self.jeorgiai_esquerda, (self.bixo_x - 26, self.bixo_y - 32),
                          (self.x_animacao_jeorgia, 0, 44, 62))
                if self.cont_animacao == 10:
                    if self.x_animacao_jeorgia == 270:
                        self.x_animacao_jeorgia = 0
                        self.cont_animacao = 0
...
class Transisao():
    def __init__(self):
        ...
        self.randomizador_jeorgia = [200,300,400,500]
        self.menu = True
        self.jogar = True
        self.creditos = False
    ...
    def iniciar (self):
        self.menu = False
        self.f_1 = True
    def Nivel_3(self,jorge_1,jeorgia_1,jeorgia_2,roberto_1,roberto_2,jogador_x,jogador_y):
...
def tela_menu(tela):
    tela.blit(menui,(0,0))
    pygame.draw.rect(tela, vermelho, (190,570,535,100),1)

    if nivel.jogar == True:
        pygame.draw.rect(tela,verde,(60,380,720,153),1)
        keys = pygame.key.get_pressed()
        ...
        pygame.draw.rect(tela, vermelho, (60, 380, 720, 153), 1)
    if nivel.creditos == True:
        pygame.draw.rect(tela, verde, (190, 570, 535, 100), 1)
        keys = pygame.key.get_pressed()
        if keys [pygame.K_w]:
...
def segunda_fase(tela, fase_1, personagem,combate):
    ...
    nivel.Nivel_3(nivel.jorge[0].vida_inimigo,nivel.jeorgia[0].vida_inimigo,nivel.jeorgia[1].vida_inimigo,
                  nivel.roberto[0].vida_inimigo,nivel.roberto[1].vida_inimigo,personagem.jogador_x, personagem.jogador_y)
